...
export function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Link href={`/product/${product.id}`} className="group block h-full" style={{ perspective: 1000 }}>
        <motion.div
          ref={ref}
        ...
        >
          ...
          <div className="flex flex-1 flex-col p-4" style={{ transform: 'translateZ(20px)' }}>
            <h3 className="line-clamp-2 min-h-[2.75rem] font-display text-base font-semibold leading-snug text-rose-900">
              {product.name}
            </h3>
            <p className="mt-1 line-clamp-2 min-h-[2rem] text-xs leading-relaxed text-rose-600/70">
              {product.description || '\u00A0'}
            </p>
            <p className="mt-auto pt-3 font-display text-lg font-bold text-rose-700">{formatPrice(product.price)}</p>
          </div>
        </motion.div>
      </Link>
    </motion.div>
